...
values  ('OPT20110507','Rahul','kumar','sharma','2001-02-25','2021-05-05'),
		...
		create view v2 as(
		select t_emp.Emp_id as Emp_id ,concat_ws(' ',Emp_f_name,Emp_m_name,Emp_l_name) as [Employee full name],
		case
       ...
       from t_emp left join v1 on
       t_emp.Emp_id=v1.Emp_id
	    ...
	    select tattend.Emp_id as Emp_id,sum(Attended_hrs) over (partition by Emp_id) as [total hours worked], tattend.Activity_id as[activity performed],Activity_description,Attended_hrs
	    from t_attendence tattend left join t_activity tactivity on tattend.Activity_id=tactivity.Activity_id
		
		)

		create view v4 as (
		select v2.Emp_id,[Employee full name], prevsalary,currentsalry,[total hours worked],[activity performed], Activity_description,Attended_hrs
		from v2 left join v3 on v2.Emp_id=v3.Emp_id
